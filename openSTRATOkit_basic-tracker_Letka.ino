...
void sendData() {
  ...
  if (gps.location.isUpdated() && gps.altitude.isUpdated()) {
    ...
    datastring_Letka = datastring;
    Temperature.requestTemperatures();
    for (uint8_t i = 0; i < nSensors; i++){
      //datastring_Letka += String(i + 1);
      //datastring_Letka += ":";
      datastring_Letka += String(Sensors.getTempCByIndex(i)) + ",";
    }
    if(ccs.available()){
      if(!ccs.readData()){
        //datastring_Letka += "CO2: ";
        datastring_Letka += String(ccs.geteCO2());
        //datastring_Letka += "/*ppm, TVOC:";
        datastring_Letka += ",";
        datastring_Letka += String(ccs.getTVOC());
      }
      else{
...
float getUV(){
	int 3v3=manalogRead(A1);
	int val=manalogRead(A0);
	float tension=3.3 / 3v3 * val;
...
>>>>>>> 1cfbafe1a96cfa0a2bdce4710886b2609eef44f5
